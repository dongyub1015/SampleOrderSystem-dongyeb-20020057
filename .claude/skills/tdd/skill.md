...
digraph tdd_cycle {
    rankdir=LR;
    plan [label="Plan.md\n테스트 계획 작성", shape=box, style=filled, fillcolor="#ffe0cc"];
    human_red [label="사람 검토\n(RED)", shape=diamond, style=filled, fillcolor="#ff9999"];
    ...
    human_review [label="사람 검토\n(REVIEW)", shape=diamond, style=filled, fillcolor="#9999ff"];
...
**RED 단계는 반드시 사람의 검토를 거쳐야 한다.**

#### 1단계: Plan.md 작성 (사람 검토 전)

다음 내용을 담은 `Plan.md`를 작성하라:
- 이번 싸이클에서 검증할 동작(behavior) 1가지
- 테스트 이름 (명확한 동작 설명)
- 테스트 입력값과 기대 결과
- 테스트 작성 이유

**Plan.md 작성 후 반드시 사람에게 검토를 요청하라. 승인 전에 테스트를 작성하지 말라.**

#### 2단계: 승인 후 테스트 작성

사람의 승인을 받은 후, 계획대로 테스트를 작성하라.
...
**REVIEW 단계는 반드시 사람의 검토를 거쳐야 한다.**

그린 상태가 된 후, 다음을 검토하라:
- 중복이 있는가? → 제거
- 이름이 명확한가? → 개선
- 헬퍼(helper) 추출이 필요한가? → 추출

테스트는 그린으로 유지하라. 동작을 추가하지 말라.

**REVIEW 검토 내용을 정리한 후, 반드시 사람에게 검토를 요청하라. 승인 전에 다음 싸이클로 넘어가지 말라.**

### 반복

사람의 승인 후, 다음 기능에 대한 Plan.md를 작성하고 새 싸이클을 시작하라.
...
사람에게 검토를 요청하고, 승인 후 다음 싸이클을 시작하라.
...
- RED 단계: Plan.md 작성 → 사람 검토 → 승인 후 테스트 작성
- REVIEW 단계: 코드 검토 → 사람 검토 → 승인 후 다음 싸이클

RED, REVIEW 단계에서 사람의 검토 없이 진행하는 것은 절대 금지다.
사람 파트너의 허가 없이는 예외 없음.s